...
@section('content')
    <div class="container-ct">
        <div class="content-left">
            <h1>Nạp Tiền ALIPAY</h1>
            <div class="data-advise">
                <div class="data-first">
                    <div class="box_news">
                        <div class="img_news"><img src="./image/{{ $new_Recharge->img }}" alt=""></div>
                        <div class="title_date">
                          <div class="date">

                            @php
                                    $time = explode(' ', $new_Recharge->created_at);
                                    $a = explode('-', $time[0]);
                                    $day_rs = $a[2];
                                    $month_rs = $a[1];
                            @endphp 
                          ...
                          <div class="title">
                            <a href="{{ asset('tin-tuc/'.$new_Recharge->id) }}" >{{ $new_Recharge->title }}</a>
                          </div>
                        </div>
                        <div class="content">
                          <span>{{ $new_Recharge->content }}</span>
                        </div>
                        <div class="view_more"><span>Đọc tiếp </span><i class="fa fa-long-arrow-right" style="color:#1030e6"></i></div>
                    </div>
                </div>
                <div class="data-list">
                    @foreach($new_Recharge_child as $new)
                    <div class="data-news">
                        <div class="image">
